...
============================================================*/
SELECT * FROM ALL_TABLES T ORDER BY T.OWNER, T.TABLE_NAME;
SELECT * FROM ALL_TAB_COLUMNS C
--WHERE C.OWNER = 'OWNER_NAME' AND LOWER(C.TABLE_NAME) LIKE LOWER('%ABC%')
WHERE C.OWNER = 'OWNER_NAME' AND LOWER(C.COLUMN_NAME) LIKE LOWER('%CTEST%')
...
WHERE TO_CHAR(SYSDATE,'YYYY-MM-DD') = TO_CHAR(TBL.UPDATED_DATE,'YYYY-MM-DD');
...
/*------------------------------------------------------------
||	NAME		    :	Fetching only n number of records from DB
||	DESCRIPTION	:	This example: number of record = 100, page number = 2
||	CREATE		  :	2024-06-02
||	LAST UPDATE	:	2024-06-02
------------------------------------------------------------*/
SELECT USER_ROLE, COUNTER FROM USER_TABLE
  GROUP BY USER_ROLE
  ORDER BY USER_ROLE
